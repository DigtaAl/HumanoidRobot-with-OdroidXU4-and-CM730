	...
	if(pan > m_KickRightAngle && pan < m_KickLeftAngle)
		{
			...
			/*if (tilt <= -10){
				...
				cout<<"Arah Robot: "<<mag<<endl;
				if(mag <= 180){
					//Walking::GetInstance()->A_MOVE_AIM_ON = true;
					//Walking::GetInstance()->A_MOVE_AMPLITUDE = 15.0;
					//cout<<"---------------masuk muter1------------"<<endl;
						cout<<"---------------masuk muter11------------"<<endl;
						if(tilt <= -12){
							jalantempat();
							usleep(630000*4);
							Tendang();
							usleep(500000);
							Action::GetInstance()->Start(122); // 108
							usleep(6000000);
							if(DEBUG_PRINT == true)
								fprintf(stderr, "[KICK]");
								detekbal = 0;
					...
					//Walking::GetInstance()->A_MOVE_AIM_ON = true;
					//Walking::GetInstance()->A_MOVE_AMPLITUDE = -15.0;
					//cout<<"---------------masuk muter2------------"<<endl;
						cout<<"---------------masuk muter22------------"<<endl;
						if(tilt <= -12){
							jalantempat();
							usleep(630000*4);
							Tendang();
			...
			//	if(offsetY <= -15 )
				//{
  					...
  					if(m_KickBallCount >= 0)
					{
						...
						//if(offsetY <= -10 && offsetX <=-10 && pan <= 0)
						if (0 >= pan >=0)//====================================================
						{
							
							jalantempat();
							usleep(1000000);
							cout<<"KANANNNNNNNNNNN"<<endl;
							Tendang();
							usleep(500000);
							Action::GetInstance()->Start(105); // ga ada motion di nomer 50
							usleep(9000000);
							...
							usleep(30000000);
							KickBall = -1; // Right
                            //Action::GetInstance()->Start(120); // ga ada motion di nomer 50
                            //usleep(6500000);
                            //jalan();
                            //usleep(10000000);
							if(DEBUG_PRINT == true)
	...
	if(m_GoalFBStep == 0 && m_GoalRLTurn == 0 && m_FBStep == 0 && m_RLTurn == 0)
		...
		else
		{
			if(detekbal==0)
			{
				...
				cout << "LURUUUUS"<<endl;
				Walking::GetInstance()->HIP_PITCH_OFFSET = 28.5; //28.1
				//Walking::GetInstance()->X_MOVE_AMPLITUDE = 5;
				Walking::GetInstance()->Z_MOVE_AMPLITUDE = 20;
