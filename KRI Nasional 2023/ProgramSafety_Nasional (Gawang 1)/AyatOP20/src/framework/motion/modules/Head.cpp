...
bool Start=true;
int count=0;
double time_move=0.9;  // 1 setting waktu putar kepala
int move_value=4;
int pan_move=0;
int tilt_move=0;
int gerak = 1;
...
void Head::InitTracking(int start)
{
    ...
    */ 
	...
	if(move<1)move=1;
	/*
	if(move==1)// top center
	{
		//if(pan <= (m_Pan_Home - 5))tilt_move=move_value; else if (pan <= (m_Pan_Home - 3)) tilt_move=0;
		//if(pan >= (m_Pan_Home + 5))tilt_move=-move_value; else if (pan >= (m_Pan_Home + 3)) tilt_move=0;
		if(tilt > (m_TopLimit - 10))tilt_move=0; else tilt_move=move_value;
		if(tilt > (m_TopLimit - 10)) move=2; //&& pan <= (m_Pan_Home - 5) && pan >= (m_Pan_Home + 5)) move=2;

	}
	else if(move==2)//right top
	{
		tilt_move=0;
		if(pan < (m_RightLimit + 5))pan_move=0; else pan_move=-move_value;
		if(pan < (m_RightLimit + 5)) move=3;
		
	}
	 else if(move==3)//right bottom
	{
		pan_move=0;
		if(tilt < (m_BottomLimit + 5))tilt_move=0; else tilt_move=-move_value;
		if(tilt < (m_BottomLimit + 5)) move=4;
	}
	else if(move==4)//left bottom
	{
		tilt_move=0;
		if(pan > (m_LeftLimit - 5)) move=5; else pan_move=move_value;
	}
	else if(move==5)//left top
	{
		pan_move=0;
		if(tilt > (m_TopLimit - 10))tilt_move=0; else tilt_move=move_value;
		if(tilt < (m_TopLimit - 10)) move=1;
	}
	*/
	...
	else if(move==6)//right bottom
	{
		if(pan < (m_RightLimit + 3))pan_move=0; else pan_move=-move_value;
		if(tilt < (m_BottomLimit + 3))tilt_move=0; else tilt_move=-move_value;
		if(move==6 && pan < (m_RightLimit + 3) && tilt < (m_BottomLimit + 3)) move = 1;
