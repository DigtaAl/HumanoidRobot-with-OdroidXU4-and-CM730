...
BallFollower::BallFollower()
{
	...
	m_KickTopAngle 		= -5.0;
	m_KickRightAngle 	= -45.0; //awal -30
	m_KickLeftAngle 	= 45.0;  //awal 30
...
//-----------------------------------------------------
    ...
    while(true){
		...
		case INITIAL:
			//cout<<"Initial Nihhhhhhhh!!!!"<<endl;
			if(StatusCheck::kickk == 0)
            {
				fprintf(stderr, "RightKick1 \n");
                Tendang();
                Action::GetInstance()->Start(136);
                usleep(3000000);
                StatusCheck::kickk = 1;
				StatusCheck::Check(cm730);
			}
			...
			if(posX == -1.0 || posY == -1.0)
						{
							...
							if(m_NoBallCount > m_NoBallMaxCount)
							{
								...
								if(Putar==0)
								{
									...
									Walking::GetInstance()->HIP_PITCH_OFFSET = 30.0; //28.1
								...
								if (tilt <= -14)
								{
									if(m_KickBallCount >= 0)
									{
										...
										m_RLTurn = 0;
											...
											Tendang();
											usleep(500000);
											Action::GetInstance()->Start(135); // 108
					...
					if(m_GoalFBStep == 0 && m_GoalRLTurn == 0 && m_FBStep == 0 && m_RLTurn == 0)
						...
						else
						{
							...
							{
								//cout << "LURUUUUS"<<endl;
								Walking::GetInstance()->HIP_PITCH_OFFSET = 29.5; //28.1
								//Walking::GetInstance()->X_MOVE_AMPLITUDE = 5;
								Walking::GetInstance()->A_MOVE_AMPLITUDE = -1.75;
								Walking::GetInstance()->Z_MOVE_AMPLITUDE = 35;
