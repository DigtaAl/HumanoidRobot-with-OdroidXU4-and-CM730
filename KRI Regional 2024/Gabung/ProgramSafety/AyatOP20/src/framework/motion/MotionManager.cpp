...
void MotionManager::Process()
{
    ...
    if(m_CalibrationStatus == 1 && m_Enabled == true)
    {
        ...
        if(m_CM730->m_BulkReadData[CM730::ID_CM].error == 0)
        {
            ...
            rl_array[buf_idx] = MotionStatus::RL_ACCEL;
            if(++buf_idx >= ACCEL_WINDOW_SIZE) buf_idx = 0;
        }

        int sum = 0, sumrl = 0, avrrl = 512, avr = 512;
        for(int idx = 0; idx < ACCEL_WINDOW_SIZE; idx++){
			sum += fb_array[idx];
			sumrl += rl_array[idx];
			}
        avr = sum / ACCEL_WINDOW_SIZE;
        avrrl = sumrl / ACCEL_WINDOW_SIZE;
        ...
        else if(avrrl < MotionStatus::FALLEN_R_LIMIT)
            MotionStatus::FALLEN = RIGHT;
        else if(avrrl > MotionStatus::FALLEN_L_LIMIT)
            MotionStatus::FALLEN = LEFT;
        else
