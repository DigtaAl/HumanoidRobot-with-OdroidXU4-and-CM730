...
void Head::InitTracking(int start)
{
    ...
    */ 
	...
	if(move==1)//left top
	{
		if(pan > (m_LeftLimit - 85)) pan_move=0; else pan_move=move_value;
		if(tilt > (m_TopLimit - 10)) tilt_move=0; else tilt_move=move_value;
		if(pan > (m_LeftLimit - 85) && tilt > (m_TopLimit - 10)) move=2;

	}
	else if(move==2)//right top
	{
		if(pan < (m_RightLimit + 85))pan_move=0; else pan_move=-move_value;
		if(tilt > (m_TopLimit - 10))tilt_move=0; else tilt_move=move_value;
		if(pan < (m_RightLimit + 85) && tilt > (m_TopLimit - 10)) move=3;
		
	}
	 else if(move==3)//right center
	{
		pan_move=0;
		if(tilt > (m_Tilt_Home - 10))tilt_move=-move_value;
		if(tilt < (m_Tilt_Home + 10))tilt_move=move_value;
		if(tilt > (m_Tilt_Home - 10) && tilt < (m_Tilt_Home + 10)) move=4;
	}
	else if(move==4)//left center
	{
		tilt_move=0;
		if(pan > (m_LeftLimit - 85)) move=5; else pan_move=move_value;
	}
	else if(move==5)//left bottom
	{
		if(pan > (m_LeftLimit - 85))pan_move=0; else pan_move=move_value;
		if(tilt < (m_BottomLimit + 20))tilt_move=0; else tilt_move=-move_value;
		if(pan > (m_LeftLimit - 85) && tilt < (m_BottomLimit + 20)) move=6;
	...
	else if(move==6)//right bottom
	{
		if(pan < (m_RightLimit + 85))pan_move=0; else pan_move=-move_value;
		if(tilt < (m_BottomLimit + 20))tilt_move=0; else tilt_move=-move_value;
		if(move==6 && pan < (m_RightLimit + 85) && tilt < (m_BottomLimit + 20)) move = 1;
