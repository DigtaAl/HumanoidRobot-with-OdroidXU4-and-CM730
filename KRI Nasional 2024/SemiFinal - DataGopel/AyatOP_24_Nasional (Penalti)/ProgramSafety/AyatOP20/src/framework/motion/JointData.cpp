...
void JointData::SetEnableWeight(bool enable, bool exclusive)
{
    //SetEnable(ID_L_LEFT,    enable,exclusive );
    //SetEnable(ID_R_RIGHT,    enable,exclusive );
}

void JointData::SetEnableWeight(bool enable)
{
    SetEnableWeight(enable,true);
}
...
{
    if(angle < MX28::MIN_ANGLE) angle = MX28::MIN_ANGLE;
    else if(angle > MX28::MAX_ANGLE) angle = MX28::MAX_ANGLE;
