...
// IMU
int arahValue, arahValue2, lastArahValue2, finalArahVAlue2;
int Arah = 280;
bool check = false;
...
void loop() {
  ...
  if (check == false) {
    ...
    Serial.println(arahValue);
    digitalWrite(ledPinRX, HIGH);  // Matikan semua LED
    digitalWrite(ledPinTX, HIGH);
  } else {
    arahValue2 = euler.x();
    arahValue2 = arahValue2 - Arah;
    if (arahValue2 < 0) {
      arahValue2 = arahValue2 + 360;
    }
    Serial.println(arahValue2);
