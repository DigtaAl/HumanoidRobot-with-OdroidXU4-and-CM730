...
void StatusCheck::Check(CM730 &cm730)
{
    ...
    if(m_old_btn & BTN_MODE)
    {
        ...
        {
            m_cur_mode++;
            if(m_cur_mode >= MAX_MODE) m_cur_mode = MOTION;
        }
    ...
    if(m_old_btn & BTN_START)
    {
        if(m_is_started == 0)
        {
            ...
            if(m_cur_mode == SOCCER)
            {
                ...
                MotionManager::GetInstance()->SetEnable(true);
                m_is_started = 1;
                 fprintf(stderr, "Start Mode Soccer.. \n");
                // LinuxActionScript::PlayMP3("../../../Data/mp3/Start soccer demonstration.mp3");
                ...
                while(1)
                {
                    if(MotionManager::GetInstance()->GetCalibrationStatus() == 1)
                    {
                        ...
                        fprintf(stderr, "Calibration Success.. \n");
                        break;
                    }
            ...
            /*tif(m_cur_mode == MOTION)
            {
                MotionManager::GetInstance()->Reinitialize();
                MotionManager::GetInstance()->SetEnable(true);
                m_is_started = 1;
                 fprintf(stderr, "Start Mode Motion.. \n");
            ...
            /*else if(m_cur_mode == VISION)
            {
                ...
                MotionManager::GetInstance()->SetEnable(true);
                m_is_started = 1;
                 fprintf(stderr, "Start Mode Motion 2.. \n");
