...
bool Start=true;
int count=0;
double time_move=0.8;  // 1 setting waktu putar kepala
int move_value=4;
int stop_value=0;
int pan_move=0;
int tilt_move=0;
...
void Head::InitTracking(int start)
{
    ...
    */ 
	...
	if(Start==true){
		...
		move=start;
		end_search=move;
		if(end_search<1)end_search=move;
		Start=false;
		search_count=0;
		putar=0;
	}
	...
	if(move==1)//top
	{
		cout<<"tiltttt	:"<<tilt<<endl;
		cout<<"toppppp right	:"<<endl;
		if(pan < (m_RightLimit + 87))pan_move=0; else pan_move=-4;
		//if(pan > (m_LeftLimit - 87)) pan_move=0; else pan_move=4;
		if(tilt > (m_TopLimit - 10)) tilt_move=0; else tilt_move=4;
		if(tilt > (m_TopLimit - 10) && pan < (m_RightLimit + 87)) move=2;

	}
	else if(move==2)//bottom
	{
		cout<<"tiltttt	:"<<tilt<<endl;
		cout<<"toppp left	:"<<endl;
		//if(pan < (m_RightLimit + 87))pan_move=0; else pan_move=-4;
		if(pan > (m_LeftLimit - 87)) pan_move=0; else pan_move=4;
		if(tilt > (m_TopLimit - 10)) tilt_move=0; else tilt_move=4;
		//if(tilt < (m_BottomLimit + 10))tilt_move=0; else tilt_move=-4;
		if(tilt < (m_TopLimit + 10) && pan > (m_LeftLimit - 87)) move=3;
	...
	else if(move==3)
	{
		...
		//if(pan > (m_LeftLimit - 87)) pan_move=0; else pan_move=4;
		if(tilt < (m_BottomLimit + 10))tilt_move=0; else tilt_move=-4;
		if(tilt < (m_BottomLimit + 10) && pan < (m_RightLimit + 87)) move=4;
	}
		else if(move==4)
	{
		cout<<"tiltttt	:"<<tilt<<endl;
		...
		if(pan > (m_LeftLimit - 87)) pan_move=0; else pan_move=4;
		if(tilt < (m_BottomLimit + 10))tilt_move=0; else tilt_move=-4;
		if(tilt < (m_BottomLimit + 10) && pan > (m_LeftLimit - 87)) move=1;
	}
