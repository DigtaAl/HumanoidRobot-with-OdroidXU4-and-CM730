...
int StatusCheck::m_sprint_sub_mode = SPRINT_PLAY;
...
void StatusCheck::Check(CM730 &cm730)
{
    ...
    if(m_old_btn & BTN_MODE)
    {
        ...
        if(m_is_started != 0)
        {
            m_is_started    = 0;
            m_cur_mode      = READY;
            if(LinuxActionScript::m_is_running)
        ...
        if(m_cur_mode == READY)
        {
            cm730.WriteByte(CM730::P_LED_PANNEL, 0x01|0x02|0x04|0x05, NULL);
        ...
        else if(m_cur_mode == SPRINT)
        {
           ...
           fprintf(stderr, "siap sprint BRO!!! \n");
        }
    }
